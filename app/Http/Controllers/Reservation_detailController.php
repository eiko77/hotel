...
class Reservation_detailController extends Controller
{
    ...
    {
      return view('reservation_detail.show');
    
        $reservation=Reservation::where('reservation_id',$reservation_id);
        return view('reservation_detail.show');   
    }

 //Reservationにデータが入った時表示させたい（認証不要＿管理画面のため）
// if (isset($request->id)) {
//   $param = ['id' => 
//   $request->id];
//   $Reservation_detailItems = DB::select
//   ('select * from reservation_details where id = :id', $param);
// } else {
//   $Reservation_detailItems = DB::select('select * from reservation_details');
// }
// return view('reservation_detail.index', ['items' => $Reservation_detailItems]);
    ...
    // return view('reservation_detail.show', ['item' => $Reservation_detailItem]);
    // }
